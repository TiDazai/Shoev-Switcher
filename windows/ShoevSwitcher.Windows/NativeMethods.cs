...
internal static class NativeMethods
{
    ...
    internal static readonly nuint SpellInputMarker = unchecked((nuint)0x53484F455653504CUL);
    ...
    [StructLayout(LayoutKind.Explicit)] internal struct InputUnion { [FieldOffset(0)] internal MouseInput mouse; [FieldOffset(0)] internal KeybdInput keyboard; }
    ...
    internal static bool Replace(int deleteUnits, string replacement, string delimiter)
    {
    ...
    internal static bool SendTestText(string value)
    {
        ...
        {
            inputs.Add(Create(0, unit, KeyeventfUnicode, 0x54455354));
            inputs.Add(Create(0, unit, KeyeventfUnicode | KeyeventfKeyup, 0x54455354));
        }
    ...
    private static Input Create(ushort key, ushort scan, uint flags, nuint? marker = null) => new() { type = InputKeyboard, data = new InputUnion { keyboard = new KeybdInput { virtualKey = key, scanCode = scan, flags = flags, extraInfo = marker ?? SelfInputMarker } } };
}
...
internal sealed class KeyboardHook : IDisposable
{
    ...
    private nint Callback(int code, nint message, nint data)
    {
        if (code >= 0 && (message == NativeMethods.WmKeyDown || message == NativeMethods.WmSysKeyDown))
        {
            ...
            if (raw.extraInfo == NativeMethods.SpellInputMarker)
            {
...
internal readonly record struct KeyEvent(uint VirtualKey, uint ScanCode, bool FromShoevSpell = false)
{
